...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_jv/helpers/content.php');
...
class ContentModelArticle extends JModelAdmin
{
	...
	/**
	 * The type alias for this content type (for example, 'com_jv.article').
	 *
	 * @var      string
	 * @since    3.2
	 */
	public $typeAlias = 'com_jv.article';
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_jv.article.' . (int) $record->id);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_jv.article.' . (int) $record->id);
		}
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_jv.article');
			}
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jv.article', 'article', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_jv.article.' . (int) $id))
	...
	protected function loadFormData()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_jv.edit.article.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_jv.articles.filter');
		...
		$this->preprocessData('com_jv.article', $data);
	...
	/**
	 * Custom clean the cache of com_jv and content modules
	 *
	 * @since   1.6
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_jv');
