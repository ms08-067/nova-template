 ...
 * @since       1.6
 */
class ContentModelproducts extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		if (JLanguageAssociations::isEnabled())
		{
			$query->select('COUNT(asso2.id)>1 as association')
				->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_jv.item'))
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_jv.product')
				);
		}
