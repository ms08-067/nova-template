...
$canOrder	= $user->authorise('core.edit.state', 'com_jv.product');
$archived	= $this->state->get('filter.published') == 2 ? true : false;
$trashed	= $this->state->get('filter.published') == -2 ? true : false;
$saveOrder	= $listOrder == 'fp.ordering';
?>

<form action="<?php echo JRoute::_('index.php?option=com_jv&view=featured'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<?php else : ?>
			<table class="table table-striped" id="productList">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$assetId	= 'com_jv.product.'.$item->id;
					$canCreate	= $user->authorise('core.create',     'com_jv.category.'.$item->catid);
					$canEdit	= $user->authorise('core.edit',       'com_jv.product.'.$item->id);
					$canCheckin	= $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
					$canChange	= $user->authorise('core.edit.state', 'com_jv.product.'.$item->id) && $canCheckin;
					?>
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="nowrap has-context">
							<div class="pull-left">
								...
								<?php endif; ?>
								<?php if ($canEdit) : ?>
									<a href="<?php echo JRoute::_('index.php?option=com_jv&task=product.edit&return=featured&id=' . $item->id);?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
