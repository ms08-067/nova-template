...
abstract class JHtmlContentAdministrator
{
	...
	public static function association($articleid)
	{
		...
		if ($associations = JLanguageAssociations::getAssociations('com_jv', '#__content', 'com_jv.item', $articleid))
		{
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_jv&task=article.edit&id=' . (int) $item->id);
					$tooltipParts = array(
	...
	public static function featured($value = 0, $i, $canChange = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured',	'articles.featured',	'com_jv_UNFEATURED',	'com_jv_TOGGLE_TO_FEATURE'),
			1	=> array('featured',	'articles.unfeatured',	'com_jv_FEATURED',		'com_jv_TOGGLE_TO_UNFEATURE'),
		);
